...
# COMMIT_ID = branch-time-sha
COMMIT_ID=$(echo ${GITHUB_REF}-$(date +%Y-%m-%d-%H-%M-%S)-$(echo ${GITHUB_WORKFLOW_SHA} | cut -c1-8))
...
for DATASET in $DATASET_LIST; do
    ...
    COMMIT_ID=$COMMIT_ID DATASET=$DATASET python .github/workflows/scripts/upload_bench_results.py
done