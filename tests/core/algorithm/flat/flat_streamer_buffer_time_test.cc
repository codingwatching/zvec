...
TEST_F(FlatStreamerTest, TestLinearSearchMMap) {
  BufferManager::Instance().init(1024 * 1024 * 1024, 1);
  ...
  size_t data_cnt = 300000UL, cnt = 100UL;
...
TEST_F(FlatStreamerTest, TestLinearSearchRandom) {
  ...
  size_t data_cnt = 30000UL, cnt = 100UL;
  IndexQueryMeta qmeta(IndexMeta::DT_FP32, dim);
  for (size_t i = 0; i < data_cnt; i++) {
    NumericalVector<float> vec(dim);
    for (size_t j = 0; j < dim; ++j) {
...
// Elapsed time: 97540565 us
// Elapsed time: 9508241 us
